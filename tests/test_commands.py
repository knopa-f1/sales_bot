...
async def test_start_command_creates_user(mock_get_user, mock_save_user):
    ...
    mock_from_user.full_name = "Тестовый Пользователь"
    ...
    mock_get_user.return_value = MagicMock(name="UserMock", name_field="Тестовый Пользователь")

    await process_start_command(mock_message, mock_bot)

    mock_save_user.assert_called_once_with(chat_id=123456789, name="Тестовый Пользователь")
